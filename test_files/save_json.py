...
def main():
	...
	while True:
			...
			json_data = r.text
			target = open("all_games_%s.json" % i, 'w')
			target.write(json_data.encode('utf-8'))
			target.close()
			...
			for key in data:
				if key == 'games':
					for game_info in data[key]:
						...
						target = open('%d_%s.json' % (gc_id, i), 'w')
						target.write(json_data_game.encode('utf-8'))
						target.close()
						...
						print 'Home Team Score - %s' % home_team_name
						print h_dict['tot']['g']
						home_team_score = h_dict['tot']['g']
						print 'Home: %s' % home_team_score

						print 'Away Team Score - %s' % away_team_name
						print a_dict['tot']['g']
						away_team_score = a_dict['tot']['g']
						print 'Away: %s' % away_team_score
...
def set_arguments(argv):
	...
	for opt, arg in opts:                
			...
			if arg in team_list:
				...
				print "Sample team:", nhl_team_dict[arg]
